...
int findString(char *str, char *substr)
{
    ...
    while(*p1 != 0) {
        ...
        if(*p1 == *p2) {
         ...
         if(*p2 == 0) return j - str - 1; // -1 to 0-index it
         else { p1 = j; p2 = substr; } // reset
        }
        p1++;
    }
...
void insertString(char *str, char *insertStr, int loc)
{
    ...
    int len2 = 0, i=0;
    while(*(insertStr+len2) != 0) len2++; // finds the length of the string we're inserting
    while(*str != 0) str++; // advance to the end
    
    while(str >= insertionPoint) { // working backwards, move characters over
        *(str + len2) = *str--;
    }
